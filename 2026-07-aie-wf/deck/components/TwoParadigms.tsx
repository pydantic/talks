...
function Box({
	...
	return (
		...
		>
			...
			<div style={{ flex: 1 }}>
				{intro ? (
					<div style={{ fontSize: "1.25rem", marginBottom: "0.6rem" }}>
						{intro}
					</div>
				) : null}
				<ul
...
export default function TwoParadigms({
	variant,
}: {
	variant: "tasks" | "env";
}) {
	...
	return (
		...
		>
			<Box
				...
				footerLabel={
					variant === "env" ? "typical boot time" : "typical duration"
				}
				footerValue={variant === "env" ? "1-3s" : "milliseconds"}
			/>
			<Box
				title="Heavy tasks"
				color={HEAVY}
				...
				footerValue={variant === "env" ? "1-3s" : "minutes"}
			/>
		</div>
	);
}
