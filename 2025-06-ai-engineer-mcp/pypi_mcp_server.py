...
async def pypi_downloads(question: str, ctx: Context[ServerSession, None]) -> str:
    """Analyze downloads from the Python package index PyPI to answer questions about package downloads."""
    result = await agent.run(question, model=MCPSamplingModel(session=ctx.session))
    return format_as_xml(result.output, item_tag='row', include_root_tag=False)
