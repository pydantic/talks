...
export default function SandboxContinuum({ showMonty = true }: { showMonty?: boolean }) {
  return (
    <svg
    ...
    >
      ...
      {Object.values(X).filter((x) => showMonty || x !== X.monty).map((x) => (
        <line
          key={x}
          x1={x}
          y1={Y_HIGH}
      ...
      {showMonty && <SolutionLabel x={X.monty} lines={['Monty']} highlight />}
      ...
      {showMonty && <Dot x={X.monty} y={465} color={COLORS.price} />}
