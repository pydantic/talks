...
export default function HeapVmDiagram() {
  ...
  // Snapshot from `(1 + 1) * "hello"`, IP at `Mul`:
  //   stack = [Str 'hello' (top, a heap ref), Int 2 (the Add result, inline)]
  //   heap  = [Str 'hello' rc=1, (Mul will allocate 'hellohello')]
  // Ints are inline immediates on the stack — only the Str needs the heap.
  return (
    <div style={wrap}>
      <svg viewBox="0 0 640 340" style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        <BytecodeRow x={22} y={172} op="LoadConst" arg={'"hello"'} faded />
        ...
        <StackCell x={212} y={62} w={138} h={48} type="Str (top)" value="'hello'" hint="ref →" />
        <StackCell x={212} y={120} w={138} h={48} type="Int" value="2" hint="inline" />
        ...
        {/* Heap holds reference types only — Ints are inline on the stack */}
        <HeapEntry x={392} y={50} w={226} h={62} type="Str" value="'hello'" rc="1" />

        {/* Mul is about to allocate the result string on the heap */}
        <g opacity={0.4}>
          <rect x={392} y={132} width={226} height={62} rx={6} fill="none" stroke={HEAP_STROKE} strokeWidth={1.2} strokeDasharray="4 4" />
          <text x={404} y={152} fontSize={10} fill={MUTED} fontFamily="var(--font-mono)">Str</text>
          <text x={608} y={152} fontSize={9} fill={MUTED} textAnchor="end" fontFamily="var(--font-mono)">after Mul</text>
          <text x={404} y={174} fontSize={16} fill={HEAD} fontFamily="var(--font-mono)">'hellohello'</text>
        </g>

        {/* ARROW: only the Str is a heap reference; the Int is inline */}
        <path
          d="M 350 86 C 372 86, 380 82, 392 80"
          fill="none"
          stroke={ARROW}
          strokeWidth={1.6}
